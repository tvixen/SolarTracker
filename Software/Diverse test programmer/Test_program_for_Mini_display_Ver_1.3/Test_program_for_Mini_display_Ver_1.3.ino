...
// Test_program_for_Mini_display_Ver_1.3 passer til Soltracker_til_MiniPanel_RF_ver_1.3
// Update 15/07-2016
// Kode er nu lavet klar og porteres til Soltracker_til_Mini_Display_ver_2.0
...
const byte ButtonMotorEWOn     = 46;  // fjeder kontakt nr 2 fra højre on=46. (45 bruges som +5v og sættes HIGH)
...
const byte ButtonPanelOn       = 4;   // test kontakt
...
int Panelsize             = 0;         // Set which panel we are using. See setup()
...
bool ButtonPanelState         = 0; //Active LOW. Select Panel model
...
uint64_t pipes[2] = { 0xDEDEDEDEE1LL, 0xDEDEDEDEE2LL };  //WritingPipe, ReadingPipe
...
void setup(void) 
{
 //Initialize ports and variables  
  ...
  pinMode(ButtonPanelOn,       INPUT_PULLUP);  // test knap til panel model          
  ...
  if ( RTC.readRTC(DS1302_TRICKLE) == DS1302_TCR_INI )   //Check the trinkler if its disabled. Can only be enabled if the battery is LiPo.
    ...
    Serial.println("Version: Test_program_for_Mini_display_Ver_1.3");
 ...
 //Read spring contact. Tell software what panel type we are using
   ButtonPanelState = digitalRead(ButtonPanelOn); //Fjederkontakt
   delay(100);        // stabilize read of contact
   if (ButtonPanelState == 0) //if run er holdt nede ved start skal der benyttes denne kode
      {  
        pipes[2]           =  0xDEDEDEDEE7LL, 0xDEDEDEDEE9LL; //("for BigPanel")
        Serial.println("Talking to Big Panel");
        Panelsize=1;  // Tell software what panel type we are using
      }  
   else
      {  
        pipes[2]           =  0xDEDEDEDEE1LL, 0xDEDEDEDEE2LL; //WritingPipe, ReadingPipe  (For SmallPanel)
        Serial.println("Talking to Mini Panel");
        Panelsize=2;  // Tell software what panel type we are using
      }        
   delay(1000);       // time to read the display 
