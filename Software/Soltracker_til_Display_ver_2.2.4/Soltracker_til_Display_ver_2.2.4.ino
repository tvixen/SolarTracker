...
void setup(void) 
{
 //Initialize ports and variables  
  ...
  if ( RTC.readRTC(DS1302_TRICKLE) == DS1302_TCR_INI )   //Check the trinkler if its disabled. Can only be enabled if the battery is LiPo.
    ...
    Serial.println("Version: SolTracker_til_Display_ver 2.2.4");
  ...
  if ((Prog == 0) && (ProgSeconds + 10) >= second(t)) // er der gået mindre end 10 sek kan der programmeres
   {
    switch (Menuvalg) //Read the value of potmeter to chose submenu
    {
     ...
     case 3:
        ...
        //setTime(18, 10, 10, 13, 12, 2018); //Ret tiden her, hvis det går helt galt.
...
*/     default:
   ...
   u8g.drawFrame(0,92,205,12);   //Ramme om den nye værdi øverste venstre hjørne X, øverste venstre hjørne Y, længde, bredde              
//   u8g.setFont(u8g_font_timR08); 
   u8g.setPrintPos(3,102);
   u8g.print("EU");
   u8g.setPrintPos(18,102);
   u8g.print(lightSensorEastUpValue);  

   u8g.setPrintPos(43,102);
   u8g.print("WU");
   u8g.setPrintPos(62,102);
   u8g.print(lightSensorWestUpValue);

   u8g.setPrintPos(85,102);
   u8g.print("ED");
   u8g.setPrintPos(100,102);
   u8g.print(lightSensorEastDownValue);

   u8g.setPrintPos(122,102);
   u8g.print("WD");
   u8g.setPrintPos(140,102);
   u8g.print(lightSensorWestDownValue);

   u8g.setPrintPos(162,102);
   u8g.print("AVR");
   u8g.setPrintPos(186,102); // Disse 2 kan også bruges til noget nyttigt
...
//   char VoltLine[232]    = "";        // Set of values to show a graph for Volts
  ...
  if (counter < 232) // bruges til at putte i rigtige plads i arrayet
    {
      counter ++;
      AmpLine[counter]=lightSensorWestDownValue;
//      Ampline shift left
    }
  else
    {  
      counter=0;
      AmpLine[counter]=Amps;
    }

    float AmpsLive  = 0.0;

  //int i;
  // Amps= atof(AmpLine[i]);        // konverterer over i Voltage som er en float (der kan laves beregninger på)


                      val = strtok (AmpLine,","); //Hiver den første variable ud af stringen
                      AmpsLive= atof(val);        // konverterer over i Voltage som er en float (der kan laves beregninger på)
                      while ((val = strtok (NULL, ",")) != NULL)
                       {
                        ++LCDcount;
                        switch (LCDcount)
                         {
                          case 1: AmpsLive = atof(val);
                          break;
                         } //switch
                       } //while 
...
int wwi;
 ...
 for (wwi = 3; wwi < 232; wwi++) 
    {  
      u8g.drawPixel(wwi,lightSensorWestDownValue);
      
      u8g.drawPixel(wwi,116);
    }
//     u8g.drawPixel(AmpsLive,i);

  
   
   u8g.setPrintPos(2,80);
   u8g.print("Amp");u8g.print(wwi);
   u8g.drawPixel(lightSensorEastDownValue,80);
   

   u8g.setPrintPos(2,110);
   u8g.print("Volt");
   u8g.drawPixel(lightSensorWestDownValue,110);
...
{ 
  ...
  if (WifiOn==3)
   ...
   // Show version
   u8g.drawStr( 1, 80, "Version 2.2.4           ");     
